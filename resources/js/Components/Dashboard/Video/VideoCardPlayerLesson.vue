...
const isLastOfCourse = computed(() =>
  String(props.video?.id ?? '') === String(props.lastVideoIdLesson ?? '')
)

const isLastOfLesson = computed(() =>
  String(props.video?.id ?? '') === String(props.lastVideoId ?? '')
)
...
onMounted(() => {
  ...
  player.on('ended', () => {
    registerVideoActivity('ended', player.currentTime())
