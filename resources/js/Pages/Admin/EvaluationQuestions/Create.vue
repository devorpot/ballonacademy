...
const form = useForm({
  type: 0, // 0 = opción múltiple, 1 = abierta
  question: '',
  response_text: '',
  response_option: null,
  status: true,
  points: props.question?.points ?? 1, // en Edit
});
...
const handleTypeChange = () => {
  if (form.type == 0 && options.value.length < 2) {
    while (options.value.length < 2)
      options.value.push({ id: nextOptionId, value: nextOptionId, label: '' }), nextOptionId++;
  }
  if (form.type == 1) {
    form.response_option = null;
  }
...
const submit = () => {
  form.transform((data) => ({
    ...
    options_json: isMultiple.value
      ...
      : [],
  })).post(route('admin.evaluation-questions.store', props.evaluation.id), {
    preserveScroll: true,
    onSuccess: () => {
      toast.value = { message: 'Pregunta creada correctamente', type: 'success' };
...
<template>
  <Head :title="`Nueva Pregunta | Evaluación #${evaluation.id}`" />
  <AdminLayout>
    <Breadcrumbs
      username="admin"
      :breadcrumbs="[
