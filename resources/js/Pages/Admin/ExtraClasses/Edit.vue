                  ...
                  <div class="col-12 col-md-9">
                    <div class="row">
                      ...
                      <div class="col-12 mb-3" v-else>
                        ...
                        />
                        <small v-if="hasExistingVideo" class="text-muted d-block mt-1">
                          Video actual: disponible en el servidor. Si subes uno nuevo, reemplazará al existente.
                        </small>
                      </div>
                      ...
                      <div class="col-md-4 mb-12" v-if="Number(form.is_youtube) === 2">
                        ...
                        />
                        <small v-if="hasExistingSubs" class="text-muted d-block mt-1">
                          Subtítulos actuales presentes. Al cargar uno nuevo, se reemplaza.
                        </small>
                      </div>
...
// Previews
const imagePreview = props.extra?.image ? `/storage/${props.extra.image}` : null
const coverPreview = props.extra?.cover ? `/storage/${props.extra.cover}` : null

// Flags para removals opcionales
...
const form = useForm({
  ...
  extract: props.extra.extract ?? '',
  description: props.extra.description ?? '',

  image: null,
  cover: null,

  is_youtube: Number(props.extra.is_youtube ?? 1),
  youtube_url: props.extra.youtube_url ?? '',
  video: null,
  subt_str: null,

  category: props.extra.category ?? '',
  tags: props.extra.tags ?? '',
  active: Number(props.extra.active ?? 1),
  order: Number(props.extra.order ?? 0),

  // Opcionales para backend si soportas borrar
  remove_image: false,
  remove_cover: false
})
...
// Estado de archivos existentes (para hints)
...
const validateField = (field) => {
  switch (field) {
    ...
    case 'video':
      // En edición, solo obligatorio si origen es archivo y no existe uno previo y no suben nuevo
      if (Number(form.is_youtube) === 2 && !hasExistingVideo.value && !form.video) {
...
const submit = () => {
  ...
  form
    ...
    .post(route('admin.extras.update', props.extra.id), {
      forceFormData: true,
      preserveScroll: true,
      onError: (errors) => {
